...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function cargar() {
      ...
      const resumenAgregado = resumenes?.reduce((acc: any, d: any) => ({
        total_dia: (acc.total_dia || 0) + (d.total_dia || 0),
        cantidad_tickets: (acc.cantidad_tickets || 0) + (d.cantidad_tickets || 0),
        efectivo: (acc.efectivo || 0) + (d.efectivo || 0),
        tarjeta: (acc.tarjeta || 0) + (d.tarjeta || 0),
        transferencia: (acc.transferencia || 0) + (d.transferencia || 0),
        mercadopago: (acc.mercadopago || 0) + (d.mercadopago || 0),
        pedidosya: (acc.pedidosya || 0) + (d.pedidosya || 0),
      }), {}) || null
  ...
  const kpis = [
    { label: 'Total', value: fmt(hoy?.total_dia || 0), color: '#4ADE80', bg: '#4ADE8010', border: '#4ADE8025' },
          ...
          <>
            ...
            <div className="grid-2">
              ...
              <div style={{ background: c.card, border: `1px solid ${c.border}`, borderRadius: '16px', padding: '1.25rem' }}>
                ...
                {medios.length === 0 ? (
                  <div style={{ color: c.muted2, fontSize: '0.85rem', textAlign: 'center', padding: '2rem 0' }}>Sin ventas</div>
                ) : (
